...
import { ISearchedProfile } from '../types'

type IState = ICommunityState

export const createCommunitySlice =
    (searchedProfiles: ISearchedProfile[]): StateCreator<IState, [], [], ICommunityState> =>
    (set) => ({
        community: {
            ...
            filters: {
                ...
                unsetServer: () => {
                    set((state) =>
                        R.assocPath(['community', 'filters', 'server'], undefined, state),
                    )
                },
                ...
                unsetVoice: () => {
                    set((state) =>
                        R.assocPath(['community', 'filters', 'usesVoice'], undefined, state),
                    )
                },
