...
import { IGame, IShopOrder, IShopProduct } from '@/store/types'
import { useEffect, useMemo, useState } from 'react'
import { Navigate, useParams } from 'react-router-dom'
import { cn } from 'tailwind-cn'

const GameShop = ({ game }: { game: IGame }) => {
    const productsIndexed = useStore((state) => state.products.items)
    const products = useMemo(
        () => Object.values(productsIndexed).filter((i) => i.gameId === game._id),
        [productsIndexed, game],
    )
...
export const GameShopScreen = () => {
    ...
    const game = useStore((state) => (slug ? state.games.items[slug] : null))

    if (game) {
        return <GameShop game={game} />
    } else {
        return <Navigate to={buildAppPath('/shop')} />
    }
}
