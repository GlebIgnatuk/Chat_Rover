...
const createCanvas = (width, height, ratio = 1, setTransform = true) => {
    ...
    const Drawing = {
        mask: (drawMask, ...drawShapes) => {
            const mask = createCanvas(width, height, ratio)
            const shape = createCanvas(width, height, ratio)
        ...
        textV2: (text, x, y, options) => {
            ...
            if (typeof options.lineHeight === 'string') {
                ...
                lineHeight = (options.lineHeight ?? 1) * (Math.abs(measuredText.fontBoundingBoxAscent) + Math.abs(measuredText.fontBoundingBoxDescent))
            }
            ...
            y += lineHeight - (lineHeight - (Math.abs(measuredText.actualBoundingBoxAscent) + Math.abs(measuredText.alphabeticBaseline))) / 2
                ...
                if (options.maxWidth === undefined) {
                    ...
                    for (let i = 1; i < words.length; ++i) {
                        ...
                        if (measurements.width < (options.maxWidth ?? Infinity)) {
                            ...
                            if (i !== words.length - 1 && lineHeight * (lines.length + 1) > (options.maxHeight ?? Infinity)) {
                                overflewY = true
                    ...
                    if (overflewY === false && lineHeight * (lines.length + 1) > (options.maxHeight ?? Infinity)) {                            
                        const ellipsisWidth = ctx.measureText('…').width
